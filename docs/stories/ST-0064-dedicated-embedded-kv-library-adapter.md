...
([ST-0062](ST-0062-kv-store-port.md)) — a real option, but not the v1
...
1. Passes the KV-store Port's conformance suite
   ([ST-0062](ST-0062-kv-store-port.md)) like any other Adapter, when
   built.
2. Documents its trade-offs against the default (separate storage file
   ...
   app-database writes).
3. Satisfies [ST-0062](ST-0062-kv-store-port.md)'s expiry contract
...
[ST-0062](ST-0062-kv-store-port.md).
