...
The FastAPI/ASGI HTTP surface [EP-0006](../epics/EP-0006-refinement-web-ui.md)'s
...
([DEC-0202](../decisions/DEC-0202-fastapi-selected.md)). Session turn
streaming is a separate story
([ST-0059](ST-0059-inbound-api-session-sse-streaming.md)) per the
...
1. HTTP routes serve: Business Goal/Epic/Story/Spike artifact reads
   ...
   (per [ST-0045](ST-0045-goal-artifact-view.md)); gate actions
   (approve/request-changes) that drive the host PR through the
   connector (per [ST-0046](ST-0046-goal-gate-surface.md)); conflict
   reads (per [ST-0047](ST-0047-minimal-conflict-view.md)); and
   notification reads, read/unread updates, and preference writes for
   the authenticated participant
   (per [ST-0048](ST-0048-notification-center.md)).
2. Every route requires an authenticated participant, resolved through
   [ST-0022](ST-0022-identity-auth-and-person-resolution.md)'s
...
5. Governance dashboard/metrics routes expose
   [ST-0018](ST-0018-governance-event-log-metrics.md)'s existing
   metrics/query API over HTTP as fixed named endpoints — never a
   generic query language at the HTTP boundary, preserving
   [DEC-0163](../decisions/DEC-0163-governance-metrics-api-named-endpoints.md)'s
   shape; this story computes nothing itself — it fronts the existing
   API (per [DEC-0042](../decisions/DEC-0042-governance-reporting-split.md)).
6. The route surface is documented as a language-neutral contract
   ...
   requirement (per [DEC-0018](../decisions/DEC-0018-python-backend-language-agnostic-specs.md)).
...
- SSE session-turn streaming —
  [ST-0059](ST-0059-inbound-api-session-sse-streaming.md).
- Gate/governance decision logic itself —
  [EP-0003](../epics/EP-0003-governance-and-gate-engine.md); this
...
- Governance metrics computation — computed by
  [ST-0018](ST-0018-governance-event-log-metrics.md); this story only
  exposes it over HTTP.

## Notes for Implementers

[ST-0018](ST-0018-governance-event-log-metrics.md)'s metrics/query API
predates [DEC-0127](../decisions/DEC-0127-problem-json-error-model.md)'s
