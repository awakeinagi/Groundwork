...
- `CompositionRoot.A-2` — `startup()` / `shutdown()`: the deterministic
  ...
  CMP-0011's ASGI lifespan (forward-declared
  per
  DEC-0132),
...
- `CompositionRoot.B-2` — for Queue and KV-store, binds each Port's
  ...
  (CMP-0012) and the app-database-reuse KV
  table (CMP-0014); no other Adapter is
...
- CMP-0012 — the Queue Port contract; the Root
  binds its v1 durable Adapter (per
  DEC-0204).
- CMP-0014 — the KV-store Port contract; the
...
- The Port *contracts* themselves — owned by
  ...
  and CMP-0003/CMP-0012/CMP-0014;
