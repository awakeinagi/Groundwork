...
overview: >-
  ...
  Hit live during SES-0071/SP-0016's Findings/Resulting Decisions
  write-up: content was mistakenly authored with the section's own
  heading line included, and a corrective retry (heading stripped)
  did not fix it -- it inserted a second full copy ahead of the
  stranded phantom-headed original, leaving both sections duplicated
  in docs/spikes/SP-0016-reactive-hook-loop-economics.md with no
  sanctioned repair. This is a recurring defect, not a one-off:
  SP-0015 (approved, closed at SES-0064) independently carries the
  same signature -- a duplicated "## Resulting Decisions" heading
  with one shared body -- caught with the identical signature pre-
  commit on SP-0016. Proposed fixes: (1) edit-section refuses
  content containing a heading-level line at write time, or (2) an
  occurrence- disambiguation flag / delete-section op lets an
  already-corrupted section be recovered through the write API. The
  eventual fix session should also repair SP-0015's stranded
