...
produce and validate exactly this format (DEC-0018).
...
- **Impact links** (DEC-0026):
  ...
  ranked over this graph (DEC-0027).
...
- Cross-references in body prose are **bare artifact IDs** (`DEC-0152`,
  `CMP-0009`); citation clauses take the form `(per DEC-0152,
  DEC-0239)`. An inline markdown link targeting an artifact file is an
  integrity violation; every bare ID must resolve to an existing
  artifact. References to non-artifact files (CONTEXT.md, specs, code)
  and external URLs remain markdown links
  (DEC-0242, superseding DEC-0090).
  The authoritative relationship still lives in frontmatter; body
  references carry no graph semantics. Human-navigable rendering is the
  doc viewer's job (DEC-0244), not the stored text's.
...
7. Body cross-references are bare artifact IDs and resolve
   (DEC-0242):
   every bare artifact ID in body prose (outside code spans/blocks)
   resolves to an existing artifact; an inline markdown link targeting
   an artifact file is a violation; remaining relative links
   (non-artifact files) point at existing files.
