#!/usr/bin/env python3
"""Serve the Groundwork docs for the viewer (per DEC-0244, DEC-0245).

Usage: python3 tools/serve_docs.py [port]   (default: 8420)

Serves the repository root with directory listings (which
docs/human_docs.html uses to discover artifacts) plus /api endpoints
that invoke the groundwork-design-session skill scripts (vendored
.agents copy preferred, ~/.claude install as fallback):
...
_SKILL_CANDIDATES = [
    ROOT / ".agents" / "skills" / "groundwork-design-session" / "scripts",
    Path.home() / ".claude" / "skills" / "groundwork-design-session"
    / "scripts",
]
SKILL_SCRIPTS = next((p for p in _SKILL_CANDIDATES if p.is_dir()),
                     _SKILL_CANDIDATES[0])
...
def main():
    ...
    addr = ("127.0.0.1", port)
    print(f"Groundwork docs: http://{addr[0]}:{port}/docs/human_docs.html")
